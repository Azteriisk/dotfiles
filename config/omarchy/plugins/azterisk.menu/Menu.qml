...
Item {
  ...
  Connections {
    ...
    function onValuesChanged() {
      if (localAppLibrary) localAppLibrary.appsChanged()
    }
  }
  ...
  function rebuildItemsFromSources() {
    ...
    var order = mergedMenu.itemOrder.slice()
    var gamesIdx = order.indexOf("games")
    var appsIdx = order.indexOf("apps")
    if (gamesIdx !== -1 && appsIdx !== -1 && gamesIdx !== appsIdx + 1) {
      order.splice(gamesIdx, 1)
      appsIdx = order.indexOf("apps")
      order.splice(appsIdx + 1, 0, "games")
    }
        ...
        Item {
          ...
          ListView {
            ...
            delegate: BorderSurface {
              ...
              readonly property bool isImageIcon: (row.icon && (row.icon.charAt(0) === "/" || row.icon.indexOf("file://") === 0 || row.icon.indexOf("image://") === 0)) || (row.appIcon && row.appIcon.length > 0)
              readonly property bool hasIcon: row.icon.length > 0 || row.isApp || isImageIcon
              ...
              Text {
                id: iconText
                visible: row.hasIcon && !row.isApp && (!row.isImageIcon || appIconImage.status === Image.Error)
                text: row.isImageIcon ? "󰊴" : row.icon
              ...
              Image {
                id: appIconImage
                visible: row.isApp || (row.isImageIcon && appIconImage.status !== Image.Error)
                ...
                source: row.isImageIcon
                  ? (row.icon.charAt(0) === "/" ? Util.fileUrl(row.icon)
                    : (row.icon.indexOf("file://") === 0 ? row.icon
                      : (root.appLibrary ? root.appLibrary.iconSource(row.appIcon || row.icon) : row.icon)))
