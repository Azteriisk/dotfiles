...
Item {
  ...
  function lockSystem(reason) {
    if (!root.lockEnabled) {
      logEvent("lock-system-skipped", "lock disabled in current mode")
      return
    }
  ...
  function statusJson() {
    return JSON.stringify({
      ...
      stayAwakeAllowsScreensaver: root.stayAwakeAllowsScreensaver,
  ...
  function applyStayAwake(value, persist, reason) {
    ...
    if (enabled) {
      if (!root.stayAwakeAllowsScreensaver) {
